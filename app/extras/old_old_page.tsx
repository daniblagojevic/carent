...
export default function Extras() {
    ...
    const handleExtra = (item: typeof extras[0], e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isChecked) {
            setQuantities((prev) => {
                const newQty = 1; // no negative values
                return { ...prev, [item.id]: newQty };
            });
        } else {
            setQuantities((prev) => {
                const { [item.id]: _, ...rest } = prev; // remove key `id`
                return rest;
            });
        }
    };
    ...
    const handleCartItem = () => {
        ...
        // ✅ 1. Loop through quantities: add or update
        ...
        // ✅ 2. Loop through cartItems: remove if not in quantities
    ...
    const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        //updateCartItemQuantity(item.id, newQty); // ✅ update quantity in Zustand
    };
